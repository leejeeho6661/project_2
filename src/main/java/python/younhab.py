...
imgPath = os.path.abspath("../../webapp/resources/article_img")+"\\"
print(imgPath)
    ...
    for arti in articles:
        for ar in arti.find_all('p',class_=''):
            if(ar.text.__contains__("yna.co.kr")):
                ...
                article +=str(ar)
            elif(ar.text.__contains__("특파원")):
                writer = ar.text.strip()[ar.text.strip().find("특파원")-4:ar.text.strip().find("특파원")+3]
                article +=str(ar)
            elif(ar.text.__contains__("통신원")):
                writer = ar.text.strip()[ar.text.strip().find("통신원")-4:ar.text.strip().find("통신원")+3]
                article +=str(ar)
            else:
                article +=str(ar)
...
#     print("img = "+imgfile)
    ...
    cur.execute("select article_seq.nextval from dual")
    row = cur.fetchone()
    for i in row:
        articleNum = i
    db_list = [(articleNum,"mostview",title,imgfile,writer.strip(),article,0,0,inserttime)]
    print(db_list)
    cur = con.cursor()
    cur.bindarraysize=1
    cur.executemany("insert into article values(:1, :2, :3, :4, :5, :6, :7, :8, :9)",db_list)
    con.commit() 
    if cnt==11:
        break
